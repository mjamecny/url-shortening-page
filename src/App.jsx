...
export default function App() {
  ...
  const [shortenLinks, setShortenLinks] = useState(function () {
    const storedValue = localStorage.getItem("shortenLinks")
  ...
  async function handleSubmit(e) {
    ...
    const res = await fetch(
      `https://csclub.uwaterloo.ca/~phthakka/1pt-express/addUrl?long=${link}`,
      {
        method: "POST",
        headers: {
    ...
    setShortenLinks([...shortenLinks, shortenObj])
  ...
  return (
    <>
      <header className="px-1.6 py-3.2">
        <div className="flex justify-between items-center relative lg:max-w-[1200px] mx-auto">
          <img src="logo.svg" alt="logo" />
          <svg
            className="fill-gray cursor-pointer lg:hidden"
          ...
          <nav
            ...
            } w-full flex-col lg:flex-row lg:justify-between gap-3.2 bg-dark-violet lg:bg-[#fff] absolute lg:static lg:flex bottom-[-370px] lg:bottom-[0px] left-[0px] px-2.4 py-3.2 rounded-lg text-1.8 text-[#fff] lg:text-gray font-bold z-50`}
          >
            <ul className="flex flex-col lg:flex-row gap-2.4 items-center">
              <li>
                <a
                  className="lg:hover:text-dark-violet lg:transition-colors lg:duration-300"
                  href="#"
                >
                  Features
                </a>
            ...
            <hr className="w-full text-grayish-violet lg:hidden" />
            <div className="flex flex-col lg:flex-row gap-2.4">
              <button className="text-[#fff] lg:text-gray">Login</button>
              <button className="bg-cyan text-[#fff] lg:hover:bg-cyan/70 transition-colors duration-300 rounded-full py-0.8 px-2.4">
                Sign Up
              </button>
            </div>
          </nav>
      ...
      <section className="px-1.6 pb-12.8">
        <div className="max-w-[1200px] mx-auto flex flex-col lg:flex-row-reverse gap-3.2 items-center relative">
          <img
            className="w-full"
            src="illustration-working.svg"
            alt="working illustration"
          />
          <div className="flex flex-col items-center lg:items-start gap-1.6">
            <h1 className="text-3.6 lg:text-4.4 font-bold text-very-dark-violet text-center lg:text-left leading-tight">
              More than just shorter links
            </h1>
            <p className="text-grayish-violet text-center lg:text-left">
              Build your brand’s recognition and get detailed insights on how
              your links are performing.
            </p>
            <button className="bg-cyan hover:bg-cyan/70 transition-colors duration-300 text-[#fff] font-bold rounded-full py-1.6 px-3.2">
              Get Started
            </button>
          </div>
          <div className="absolute bottom-[-200px] w-full flex flex-col gap-1.6 shadow-md">
            <div className=" bg-dark-violet bg-[url('bg-shorten-mobile.svg')] bg-cover lg:bg-[url('bg-shorten-desktop.svg')] p-2.4 lg:p-4.8 rounded-lg">
              ...
              >
                <div className="flex flex-col gap-0.8 lg:flex-1">
                  <input
                    className={`rounded-md py-1.2 px-2.4 outline-none ${
                  ...
                  />
                  {error && <p className="text-red text-1.2">{errMsg}</p>}
                </div>

                <button
                  type="submit"
                  className="bg-cyan hover:bg-cyan/70 transition-colors duration-300 text-[#fff] font-bold rounded-md py-1.2 px-3.2 lg:self-start"
                >
                  Shorten it!
                </button>
              </form>
      ...
      <section className="px-1.6 pt-9.6 pb-6.4 bg-[#F0F1F6]">
        <div className="flex flex-col gap-8 lg:max-w-[1200px] mx-auto">
          <div className="flex flex-col gap-2.4">
            {shortenLinks.length > 0 &&
              shortenLinks.map((link, i) => (
                <div
                  key={i}
                  className="bg-[#fff] p-1.6 lg:px-2.4 flex flex-col lg:flex-row lg:justify-between lg:items-center gap-1.2 shadow-md rounded-md"
                >
                  <p>{link?.long}</p>
                  <hr className="w-full text-gray lg:hidden" />
                  <div className="flex flex-col lg:flex-row lg:items-center gap-0.8 lg:gap-1.2">
                    <p className="text-cyan font-bold">{link?.short}</p>
                    <button
                      className="bg-cyan hover:bg-cyan/70 transition-colors duration-300 text-[#fff] font-bold py-1.2 lg:px-3.2 rounded-md"
          ...
          <div className="flex flex-col gap-1.6 lg:items-center">
            <h2 className="text-2.4 lg:text-3.6 font-bold text-very-dark-violet text-center">
              Advanced Statistics
            </h2>
            <p className="text-grayish-violet text-center lg:w-[60%]">
              Track how your links are performing across the web with our
              advanced statistics dashboard.
            </p>
          </div>
          <div className="flex flex-col gap-8 lg:gap-4.8 relative lg:flex-row">
            <div className="absolute left-[50%] lg:left-[0px] translate-x-[-50%] lg:translate-x-[0px] border-l-[7px] lg:border-l-[0px] lg:border-t-[7px] lg:w-full lg:top-[50%] border-cyan h-full lg:h-[0px]"></div>
            <div className="py-4.8 flex flex-col gap-1.6 bg-[#fff] rounded-md px-2.4 relative lg:h-[250px]">
              <div className="absolute left-[50%] lg:left-[20px] translate-x-[-50%] lg:translate-x-[0px] translate-y-[-50%] top-[0px] w-[72px] h-[72px] flex justify-center items-center bg-dark-violet rounded-full">
                <img
              ...
              <h3 className="font-bold text-very-dark-violet text-2 text-center lg:text-left">
                Brand Recognition
              </h3>
              <p className="text-grayish-violet text-center lg:text-left">
            ...
            <div className="py-4.8 flex flex-col gap-1.6 bg-[#fff] rounded-md px-2.4 relative lg:h-[250px] lg:mt-3.2">
              <div className="absolute left-[50%] lg:left-[20px] translate-x-[-50%] lg:translate-x-[0px] translate-y-[-50%] top-[0px] w-[72px] h-[72px] flex justify-center items-center bg-dark-violet rounded-full">
                <img
                  className="w-[36px] h-[36px]"
            ...
            <div className="py-4.8 flex flex-col gap-1.6 bg-[#fff] rounded-md px-2.4 relative lg:h-[250px] lg:mt-6.4">
              <div className="absolute left-[50%] lg:left-[20px] translate-x-[-50%] lg:translate-x-[0px] translate-y-[-50%] top-[0px] w-[72px] h-[72px] flex justify-center items-center bg-dark-violet rounded-full">
                <img
                  className="w-[36px] h-[36px]"
      ...
      <footer className="bg-very-dark-violet py-6.4">
        <div className="grid grid-cols-1 lg:grid-cols-[250px_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)] gap-4.8 lg:gap-2.4 justify-items-center lg:justify-items-start lg:max-w-[1200px] mx-auto">
          ...
          <div className="flex flex-col gap-1.6 items-center lg:items-start">
            <h4 className="text-[#fff]">Features</h4>
            <ul className="flex flex-col gap-1.2 items-center lg:items-start text-gray">
              <li>
                <a
          ...
          <div className="flex flex-col gap-1.6 items-center lg:items-start">
            <h4 className="text-[#fff]">Resources</h4>
            <ul className="flex flex-col gap-1.2 items-center lg:items-start text-gray">
              <li>
                <a
                  className="hover:text-cyan transition-colors duration-300"
          ...
          <div className="flex gap-2.4 items-center lg:items-start justify-center lg:justify-start">
            <a
              className="text-[#fff] hover:text-cyan transition-colors duration-300"
              href="#"
            >
